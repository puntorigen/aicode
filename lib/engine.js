...
`;
  ...
  // multi-step agent loop (native AI SDK tool calling).
  // - messages: [{ role: 'user'|'assistant', content }] (last entry = current task)
  // - tools: an AI SDK tool() registry
  // - maxSteps: hard cap on think->tool->observe iterations (stopWhen)
  // - onText(delta): receives streamed answer text (kept separate from the
  //   classic path's this.streamHandler so sub-agents don't pollute the answer)
  // - onEvent(part): receives { type: 'tool-call'|'tool-result'|'tool-error', ... }
  //   so the UI can render each step
  // returns { text } — the agent's final answer/report
  async runAgentLoop({ messages, system = null, tools = {}, maxSteps = 20, onText = null, onEvent = null }) {
    const { streamText, stepCountIs } = require('ai');
    const promptText = (messages || []).map((m) => (typeof m.content === 'string' ? m.content : '')).join('\n');
    return await this._withProviderFallback(promptText, 'smart', async (model, provider) => {
      const call = models.callOptions(provider, 'smart', this.config, { structured: false });
      const callExtras = {
        ...(call.providerOptions ? { providerOptions: call.providerOptions } : {}),
        ...(call.maxOutputTokens ? { maxOutputTokens: call.maxOutputTokens } : {}),
      };
      const response = streamText({
        ...
        ...(system ? { system } : {}),
        ...(this.abortSignal ? { abortSignal: this.abortSignal } : {}),
        onStepFinish: (step) => { this._recordUsage(provider, 'smart', (step && step.usage) || {}); },
        ...callExtras,
      });
      let full = '';
      for await (const part of response.fullStream) {
        switch (part.type) {
          case 'text-delta': {
            const t = part.text || '';
            full += t;
            if (onText) { try { onText(t); } catch (e) { } }
            break;
          }
          case 'tool-call':
            if (onEvent) { try { onEvent({ type: 'tool-call', toolName: part.toolName, input: part.input, toolCallId: part.toolCallId }); } catch (e) { } }
            break;
          case 'tool-result':
            if (onEvent) { try { onEvent({ type: 'tool-result', toolName: part.toolName, output: part.output, toolCallId: part.toolCallId }); } catch (e) { } }
            break;
          case 'tool-error':
            if (onEvent) { try { onEvent({ type: 'tool-error', toolName: part.toolName, error: part.error, toolCallId: part.toolCallId }); } catch (e) { } }
            break;
          case 'error':
            throw (part.error instanceof Error ? part.error : new Error(String((part.error && part.error.message) || part.error || 'agent loop error')));
          default:
            break;
        }
      }
