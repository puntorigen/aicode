...
if (file.data.file!='') {
    const input_without_file = await queryLLM(
        `The following a user's text request. I need you to extract the action that the user is requesting without trying to reply it and without mentioning any file names: ${user_prompt}`
    );
